...
1. [How `CSRF` Token work in Laravel ?]() `[TheSoftKing]`

1. [PHP তে তো তারিখ নিয়ে কাজ করার জন্য ডিফল্ট ডেট ফর্মেট আছে, তা সত্ত্বেও Carbon কেন ব্যবহার করা হয়  ?]() `(TheSoftKing)`


<br>
<br>

# Q-1 : What is the difference between `get()`, `first()`, `find()` and `pluck()` in Eloquent ?
...
# Q-2 : Difference between `var_dump()`, `print_r()`, and `dd()`
...
# Q-3 : Explain about the `Lazy Loading` and `Eager Loading` 
...
# Q-4 : In Laravel, the full system which is supported by default ? Eager or Lazy which?
...
# Q-5 : The `first()` and `get()` which initialize the class?
...
# Q-7 : In the Laravel application, what is the importance of the `bootstrap/app.php` file ?
...
# Q-10 : Explain Laravel Request Lifecycle
...
# Q-12 : Explain Dependency Injection vs Service Container
...
# Q-13 : If two classes A, B. B have data/methods. How can set it dependency injection, concept of service container ?
...
# Q-15 : Explain about `hasMany`, `hasThrough` and `polymorphic` relationship.
...
# Q-17 : What is the main merits and demerits of Dependency Injection ?
...
# Q-18 : Is ORM first or Data abstraction layer first ?
...
# Q-19 : What is Route Model Binding ?
...
# Q-20 : If so many requests hit on a app, then how many instances are created ? One time or many times ?
...
# Q-22 : If I send 1000 mails or any operation using a chunk, then if any operation fails , then will the next operation continue or not ?
...
# Q-24 : What is the purpose of using `.env` in a Laravel project ?
...
# Q-25 : What is a polymorphic relationship in Eloquent? Can you give an example of its implementation?
...
# Q-26 : How would you create and use a `Query Scope` in Laravel?
...
# Q-27 : i) What is the difference between `queue` and `event` in Laravel ? <br> ii) Can you provide a use case for each ?
...
# Q-28 : What is CSRF protection, and how does Laravel implement it?
...
# Q-29 : What are some strategies for optimizing a Laravel application’s performance?
...
# Q-30 : How do you optimize large database queries in Laravel?
...
# Q-31 : What is the difference between `bind()` and `singleton()` in the service container?
...
# Q-32 : Explain how Laravel's middleware pipeline works. How would you create and chain multiple middlewares?
...
# Q-33 : How `CSRF` Token work in Laravel ? `[TheSoftKing]`

## 🔹 CSRF কী?

**CSRF** মানে 👉 **Cross-Site Request Forgery**
বাংলায় বললে — অন্য কোনো ওয়েবসাইট বা স্ক্রিপ্ট তোমার ইউজারের হয়ে **অযাচিতভাবে অনুরোধ পাঠিয়ে ফেলা**।

### উদাহরণ:

ধরো, তুমি তোমার সাইটে লগইন করা অবস্থায় আছো:

```
https://example.com/account/delete
```

এখন কেউ যদি তোমাকে অন্য ওয়েবসাইটে গিয়ে একটা ফাঁদে ফেলে দেয় — যেখানে একটা hidden form থেকে একই URL-এ POST request পাঠানো হয়,
তাহলে তোমার অ্যাকাউন্ট মুছে যেতে পারে ❌

এটাকেই বলে **CSRF Attack**।

---

## 🔹 Laravel কিভাবে এটি প্রতিরোধ করে?

Laravel প্রত্যেকটি **form submission** বা **POST request**-এর সাথে একটি **unique CSRF token** ব্যবহার করে।

এই টোকেনটি:

* সেশন অনুযায়ী তৈরি হয় (প্রত্যেক ইউজারের জন্য আলাদা)
* সার্ভার পাশে যাচাই করা হয়
* ভুল হলে “419 Page Expired” বা “CSRF token mismatch” error দেয়

---

## 🔹 কাজের প্রক্রিয়া (How it Works)

1. **Laravel একটি টোকেন তৈরি করে এবং সেশনে সংরক্ষণ করে।**
   যখন তুমি কোনো ফর্ম রেন্ডার করো, Laravel Blade অটোমেটিকভাবে টোকেনটা ইনজেক্ট করে:
...
2. **User ফর্ম সাবমিট করলে টোকেনটি Request-এর সাথে যায়।**

3. **Middleware টোকেন যাচাই করে।**
   Laravel-এর `VerifyCsrfToken` middleware (location: `app/Http/Middleware/VerifyCsrfToken.php`)
   ইনকামিং রিকোয়েস্টের `_token` ভ্যালু মিলিয়ে দেখে সেশনে সংরক্ষিত টোকেনের সাথে।

   যদি মিল পায় ✅ → রিকোয়েস্ট প্রসেস হয়
   না মিললে ❌ → `419 | Page Expired` রেসপন্স দেয়

---

## 🔹 API রিকোয়েস্টে কী হয়?

API রিকোয়েস্টে সাধারণত CSRF টোকেন ব্যবহার করা হয় না, কারণ API-তে **stateless authentication** (যেমন Sanctum, Passport, JWT) ব্যবহৃত হয়।
তবে Laravel Sanctum SPA authentication-এ CSRF ব্যবহার হয়, যেখানে ফ্রন্টএন্ড (যেমন Vue/React) প্রথমে `/sanctum/csrf-cookie` কল করে টোকেন নেয়।

---

## 🔹 উদাহরণ (Ajax Request সহ)
...
এভাবে Laravel প্রতিটি AJAX POST রিকোয়েস্ট যাচাই করে।
...
* একজন আক্রমণকারী অন্য সাইট থেকে POST রিকোয়েস্ট পাঠিয়ে **তোমার ইউজারের সেশন ব্যবহার করে** কিছু ক্ষতিকর কাজ করতে পারবে।
* Laravel টোকেন যাচাই করে এই ঝুঁকি প্রতিরোধ করে।
...
| বিষয়                   | বর্ণনা                        |
| ---------------------- | ----------------------------- |
| পূর্ণ নাম              | Cross-Site Request Forgery    |
| Laravel টোকেন তৈরি করে | সেশন ভিত্তিক ইউনিক টোকেন      |
| Blade directive        | `@csrf`                       |
| Middleware             | `VerifyCsrfToken`             |
| ব্যর্থ হলে             | 419 Page Expired Error        |
| উদ্দেশ্য               | Unauthorized Request প্রতিরোধ |
...
# Q-34 : PHP তে তো তারিখ নিয়ে কাজ করার জন্য ডিফল্ট ডেট ফর্মেট আছে, তা সত্ত্বেও Carbon কেন ব্যবহার করা হয়  ? `(TheSoftKing)`

## 🔹 প্রথমে — PHP তে কীভাবে কাজ হয়:

PHP তে date/time নিয়ে কাজ করার জন্য আগে থেকেই আছে:
...
এটি ঠিক আছে — কিন্তু এর কিছু সীমাবদ্ধতা আছে:

* অনেক কাজ করতে **বেশি কোড লিখতে হয়** (যেমন তারিখ যোগ/বিয়োগ)
* Chain করা যায় না
* Timezone, human readable difference ইত্যাদি করা কষ্টকর
* Formatting ও parsing অনেক সময় জটিল হয়ে যায়

---

## 🔹 Carbon কী?

`Carbon` হচ্ছে **DateTime ক্লাসের একটা extended version (subclass)**, যা Laravel-এর সাথে ডিফল্টভাবে ইনস্টল থাকে (`nesbot/carbon` প্যাকেজ)।
এটা মূলত DateTime-এর উপর একটা সুন্দর, expressive, developer-friendly layer।

---

## 🔹 কেন Carbon ব্যবহার করা হয় (main reasons)

### 🧠 1. সহজ সিনট্যাক্স (Human-friendly API)

```php
echo Carbon::now(); // বর্তমান সময়
echo Carbon::now()->addDays(5); // আজ থেকে ৫ দিন পর
echo Carbon::yesterday(); // গতকাল
echo Carbon::tomorrow(); // আগামীকাল
```

এই কাজগুলো raw DateTime এ অনেক বেশি verbose হয়।

---

### 🧮 2. সহজ তারিখ যোগ-বিয়োগ (Chaining Supported)
...
👉 DateTime এ এটা করতে হলে আলাদা `modify()` মেথড ব্যবহার করতে হয়:
...
Carbon এর চেইনিং আরো পড়তে সহজ।

---

### 🕒 3. Time Difference & Human Readable Format

```php
$created = Carbon::createFromDate(2025, 10, 1);
echo $created->diffForHumans(); // "3 weeks ago"
```

PHP DateTime এ এটা manually করতে হয় — Carbon এ built-in!

---

### 🌍 4. Timezone Handle করা সহজ

```php
echo Carbon::now('Asia/Dhaka');
```

এটা দিয়ে বিভিন্ন timezone অনুযায়ী সময় রূপান্তর করা সহজ হয়।

---

### 🧩 5. Laravel এর সাথে integration

Laravel এ Carbon deep integrated:

* `$model->created_at` ও `$model->updated_at` হচ্ছে Carbon instance
  তাই তুমি সরাসরি করতে পারো:

  ```php
  $user->created_at->diffForHumans();
  ```
* Validation, formatting, mutators, casts — সব জায়গায় Carbon compatible।

---

### 📦 6. Formatting ও Parsing সহজ
...
এমন natural ভাষায় parsing PHP DateTime এ default নেই।

---

### 💡 7. Additional utility

Carbon এ অনেক ready-made helper আছে:
...
| কাজ                 | PHP DateTime              | Carbon               |
| ------------------- | ------------------------- | -------------------- |
| আজকের তারিখ         | `new DateTime()`          | `Carbon::now()`      |
| আগামীকাল            | `$d->modify('+1 day')`    | `Carbon::tomorrow()` |
| পার্থক্য দেখানো     | Manual diff code          | `diffForHumans()`    |
| Timezone            | আলাদা অবজেক্ট সেট করতে হয় | Direct param দিয়ে    |
| Laravel Integration | নেই                       | পুরোপুরি আছে         |

---

## 🔹 উপসংহার:

> **Carbon ব্যবহার করা হয় কারণ এটা DateTime-এর চেয়ে অনেক বেশি readable, expressive, এবং Laravel-এর সাথে পুরোপুরি integrated।**
